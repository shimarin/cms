...
fenced code blockの言語に `mermaid` を指定すると、Pygmentsによるシンタックスハイライトを
行わず `<pre class="mermaid">` として出力する。Mermaid JSの読み込み・初期化はテンプレート側で行う。
...
Mermaidブロックを含むページではテンプレート変数 `uses_mermaid` が `true` になる。
テンプレート側で条件付きでスクリプトを読み込む例:

```jinja2
{% if uses_mermaid | default(false) %}
<script type="module">
...
Mermaid JSのバージョン選定・CDN/ローカル配布・CSS調整はコンテンツ/テンプレート側の責務とし、
CMS本体はブロック検出とHTML出力のみを担当する。詳細は ADR-013 参照。
...
| `uses_mermaid` | Mermaid fenced code blockを含む場合 `true`（それ以外はキーなし） |
...
| `inquiry.honeypot_action` | ボット検出時の動作。 `drop` (送信せず成功を返す) または `archive` (低優先度ヘッダを付けて送信)。ハニーポットとメールアドレスブラックリストの両方に適用される |
| `inquiry.honeypot_subject_prefix` | ボット検出時に件名に付与する接頭辞（デフォルト: `[Bot]`） |
| `inquiry.email_blacklist` | ブラックリストに登録するメールアドレスまたはドメインの配列。`"foo@example.com"` で完全一致、`"@example.com"` でドメイン全体をブロック（大文字小文字無視）。POSTデータの全フィールドに対してマッチングを行う |
...
`inquiry.email_blacklist` に登録したアドレス・ドメインに一致する送信者をボットと同じ扱いにする。
POSTデータの全フィールドを対象にマッチングするため、フィールド名は問わない。
...
- フルアドレス（`"foo@example.com"`）: 完全一致
- `@` で始まるエントリ（`"@example.com"`）: そのドメイン全体をブロック
- 大文字小文字は無視
- 検出時の動作は `honeypot_action` と共通
