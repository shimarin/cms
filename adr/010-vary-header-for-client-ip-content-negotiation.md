...
### 304 レスポンスにも Vary が付く

ADR-011 で `render_md_file()` の条件付きGET判定はテンプレート描画後へ移動した。
そのためテンプレート内で `client_ip_match_any()` が呼ばれた場合は、304 レスポンスにも
通常の200レスポンスと同じ `Vary` が付与される。

ただし、テンプレートが見つからず本文HTMLだけを返すフォールバック経路では
テンプレートが実行されないため、テンプレート由来の `Vary` は発生しない。
...
`X-Forwarded-For` ベースの判定を共有キャッシュが完璧に扱う保証がないため、
リスクが残る。
